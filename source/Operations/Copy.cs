...
{
    public static bool Copy(ICoreAPI api, JsonPatch patch, JToken asset)
    {
        if (patch.Path == null)
        {
            LoggerUtil.Error(api, typeof(Operations), "Copy operation json patch does not have Path specified");
            return false;
        }

        if (patch.FromPath == null)
        {
            LoggerUtil.Error(api, typeof(Operations), "Copy operation json patch does not have FromPath specified");
            return false;
        }
